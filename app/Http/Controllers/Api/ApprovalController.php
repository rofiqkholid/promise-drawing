...
class ApprovalController extends Controller
{
    ...
    private function getApprovalLevel(User $user): int
    {
        ...
        // Note: Role ID 16 (Admin) dan Role ID 31 (ENG Leader) dianggap setara dengan ICT
        if (in_array(Role::ICT, $userRoleIds) || in_array(16, $userRoleIds) || in_array(31, $userRoleIds)) {
            return 4;
        }

        // Level 3: Approver 3
        // Sekarang murni sequential, wajib menunggu Level 2 selesai
        if (in_array(Role::APV_3, $userRoleIds)) {
            return 3;
        }
    ...
    public function filters(Request $request): JsonResponse
    {
        ...
        if ($request->filled('select2')) {
            ...
            switch ($field) {
                ...
                case 'status':
                    $all = collect([
                        // Ganti ID dan Text sesuai keinginan
                        ['id' => 'Waiting Checked',  'text' => 'Waiting Checked'],   // Dulu: Waiting L1
                        ['id' => 'Waiting Approved', 'text' => 'Waiting Approved'],  // Dulu: Waiting L2
                        ['id' => 'Waiting Legalize', 'text' => 'Waiting Legalize'],  // Dulu: Waiting L3
                        ['id' => 'Approved',         'text' => 'Approved'],          // Opsional: Approved final (L3 sudah OK)
        ...
        return response()->json([
            ...
            'statuses' => collect([
                ['name' => 'Waiting Checked'],
                ['name' => 'Waiting Approved'],
                ['name' => 'Waiting Legalize'],
                ['name' => 'Approved'],
                ['name' => 'Rejected'],
            ]),
...
CASE
    WHEN pa.decision = 'pending' AND COALESCE(pa.lvl1,0) = 0
        THEN 'Waiting Checked'
    WHEN pa.decision = 'pending'
         AND COALESCE(pa.lvl1,0) = 1
         AND COALESCE(pa.lvl2,0) = 0
        THEN 'Waiting Approved'
    WHEN pa.decision = 'pending'
         AND COALESCE(pa.lvl1,0) = 1
         AND COALESCE(pa.lvl2,0) = 1
         AND COALESCE(pa.lvl3,0) = 0
        THEN 'Waiting Legalize'
...
"),
        ...
        if ($approval) {
            if ($approval->decision === 'approved') {
                ...
                if ((int) $approval->lvl1 === 0) {
                    $approvalStatus = 'Waiting Checked';
                } elseif (
                    (int) $approval->lvl1 === 1 &&
                    (int) $approval->lvl2 === 0
                ) {
                    ...
                    $approvalStatus = 'Waiting Legalize';
                }
            }
        }
    ...
    public function approve(Request $request, string $id)
    {
        ...
        try {
            ...
            // JIKA LEVEL 1
                ...
                DB::commit();
                return response()->json(['message' => 'Checked. Waiting for Approved.']);
            }
            // JIKA LEVEL 2
            else if ($level === 2) {
                if ((int) $approval->lvl1 !== 1) {
                    DB::rollBack();
                    return response()->json(['message' => 'Checked is required first.'], 422);
                }
                DB::table('package_approvals')->where('id', $approval->id)->update([
                    'lvl2' => 1,
                    'lvl2_decided_by' => $user->id,
                ...
                DB::commit();
                return response()->json(['message' => 'Approved. Waiting for Legalize.']);
            }
            // JIKA LEVEL 3 (SEQUENTIAL MODE)
            else if ($level === 3) {
                // Sekarang wajib cek Level 2
                if ((int) $approval->lvl2 !== 1) {
                    DB::rollBack();
                    return response()->json(['message' => 'Approved is required first.'], 422);
                }
                DB::table('package_approvals')->where('id', $approval->id)->update([
                    'lvl3' => 1,
                    'lvl3_decided_by' => $user->id,
                ...
                $logNote = 'Revision approved by Level 3';
                $logLevel = 'Level 3';
            } else {
                DB::rollBack();
            ...
            // --- 2. FINALISASI DOKUMEN (Hanya jalan jika Level 3 atau Level 4 lolos ke sini) ---
            ...
            ActivityLog::create([
                ...
                'meta' => [
                    ...
                    'part_no'        => $dataInfo->part_no ?? '-',
                    'doc_type'       => $dataInfo->doc_type ?? '-',
                    'ecn_no'         => $dataInfo->ecn_no ?? '-',
                    'revision_no'    => $dataInfo->revision_no ?? '-',
                    'project_status' => $dataInfo->project_status ?? '-',
                    'approval_level' => $logLevel
                ],
            ]);
            ...
            if (!$request->boolean('confirm_without_email')) {
                try {
                    ...
                    $emailData = (array) $dataInfo;
                    $emailData['approved_at'] = Carbon::now()->format('d M Y H:i');
                    $emailData['download_url'] = route('file-manager.export.detail', ['id' => str_replace('=', '-', encrypt($revisionId))]);

                    $files = DB::table('doc_package_revision_files')->where('revision_id', $revisionId)->get(['filename', 'file_size']);
                    $emailData['files'] = $files->map(fn($f) => [
                        'name' => $f->filename,
    ...
    public function reject(Request $request, string $id)
    {
        ...
        if ($level === 0) {
            return response()->json(['message' => 'You are not authorized to reject this revision.'], 403);
        }
        ...
        if (!$dataInfo) {
            return response()->json(['message' => 'Revision data not found.'], 404);
        }
        ...
        // --- 2. VALIDASI LEVEL & PENENTUAN STATUS (SEQUENTIAL VS BYPASS) ---
        $previousStatus = 'Waiting Checked';

        // JIKA BUKAN ICT (Level 1, 2, 3 wajib taat urutan)
        if ($level !== 4) {
            if ($level === 1) {
                if ((int)$approval->lvl1 !== 0) return response()->json(['message' => 'Level 1 has already processed this.'], 403);
                $previousStatus = 'Waiting Checked';
            } elseif ($level === 2) {
                if ((int)$approval->lvl1 !== 1) return response()->json(['message' => 'Cannot reject. Waiting for Level 1 approval first.'], 403);
                if ((int)$approval->lvl2 !== 0) return response()->json(['message' => 'Level 2 has already processed this.'], 403);
                $previousStatus = 'Waiting Approved';
            } elseif ($level === 3) {
                if ((int)$approval->lvl2 !== 1) return response()->json(['message' => 'Cannot reject. Waiting for Level 2 approval first.'], 403);
                if ((int)$approval->lvl3 !== 0) return response()->json(['message' => 'Level 3 has already processed this.'], 403);
                $previousStatus = 'Waiting Legalize';
            }
        } else {
            // Logika untuk ICT (Level 4 / Bypass)
            // Menentukan label status terakhir berdasarkan kolom yang sudah terisi
            if ((int)$approval->lvl2 === 1) {
                $previousStatus = 'Waiting Legalize';
        ...
        DB::beginTransaction();
        try {
            // Update Status Revisi menjadi Rejected
            ...
            // Tandai kolom level yang relevan
            if ($level === 1) {
                $update['lvl1'] = 1;
                ...
                // Jika L3 atau ICT, kita tandai di lvl3 agar antrean di dashboard L3 bersih
                $update['lvl3'] = 1;
            ...
            ActivityLog::create([
                'scope_type'    => 'package',
                'scope_id'      => $dataInfo->package_id,
                'revision_id'   => $revisionId,
                'activity_code' => ActivityLog::REJECT,
                'user_id'       => $userId,
                'meta' => [
                    'note'            => $request->note,
                    'reject_level'    => $level === 4 ? 'ICT / L4 (Bypass)' : 'L' . $level,
    ...
    public function rollback(Request $request, string $id): JsonResponse
    {
        ...
        // --- 1. VALIDASI AKSES ROLLBACK (STRICT SEQUENTIAL VS ICT BYPASS) ---
        if ($level === 0) {
            return response()->json(['message' => 'Unauthorized.'], 403);
        }

        // Jika bukan ICT (Level 4), terapkan aturan antrean
        if ($level !== 4) {
            if ($level === 1) {
                // L1 hanya bisa rollback jika dia sudah approve (1) tapi L2 belum proses (0)
                if (!((int)$approval->lvl1 === 1 && (int)$approval->lvl2 === 0)) {
                    return response()->json(['message' => 'Cannot rollback L1: Level 2 has already processed this.'], 403);
                }
            } elseif ($level === 2) {
                // L2 hanya bisa rollback jika dia sudah approve (1) tapi L3 belum proses (0)
                if (!((int)$approval->lvl2 === 1 && (int)$approval->lvl3 === 0)) {
                    return response()->json(['message' => 'Cannot rollback L2: Level 3 has already processed this.'], 403);
                }
            } elseif ($level === 3) {
                // L3 hanya bisa rollback jika dia sudah approve (Final) tapi ingin ditarik kembali
                if ((int)$approval->lvl3 !== 1) {
                    return response()->json(['message' => 'Level 3 has not approved this revision yet.'], 403);
                }
            }
        }

        // --- 2. AMBIL DATA DETAIL UNTUK LOG ---
        $dataInfo = DB::table('doc_package_revisions as dpr')
            ->join('doc_packages as dp', 'dpr.package_id', '=', 'dp.id')
            ->join('customers as c', 'dp.customer_id', '=', 'c.id')
            ->join('models as m', 'dp.model_id', '=', 'm.id')
        ...
        try {
            ...
            // Jika yang di-rollback adalah dokumen aktif (approved & !obsolete)
            ...
            // --- 4. RESET TABEL APPROVALS BERDASARKAN LEVEL ---
            $updateData = [
                'decision'   => 'pending',
                'decided_by' => null,
                'decided_at' => null,
            ...
            ActivityLog::create([
                ...
                'user_id'       => $userId,
                'meta' => [
                    'note'            => 'Rollback by ' . ($level === 4 ? 'ICT Bypass' : 'Level ' . $level),
                    'action_status'   => 'Rollback',
                    'previous_status' => ucfirst($revision->revision_status),
                    'current_status'  => $statusTujuan,
                    'customer'        => $dataInfo->customer ?? '-',
                    'model'           => $dataInfo->model ?? '-',
                    'part_no'         => $dataInfo->part_no ?? '-',
                    'doc_type'        => $dataInfo->doc_type ?? '-',
            ...
            DB::commit();
            return response()->json(['message' => "Rollback successful. Status reset to $statusTujuan."]);
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    public function exportSummary(Request $request)
    {
        ...
        $query = DB::table('doc_package_revisions as dpr')
            ...
            // TAMBAHAN: Join ke table customer_revision_labels
        ...
        $rowsDb = $query->select([
            ...
            'dpr.ecn_no',
            'dpr.revision_no',
            'crl.label as revision_label', // TAMBAHAN: Select label revisi
            'dpr.is_finish'
        ])
        ...
        foreach ($rowsDb as $index => $r) {
            ...
            $rows[] = [
                ...
                'revision_label' => $r->revision_label ?? '-', // TAMBAHAN: Map ke array export
