...
use App\Mail\ReshareProcessedNotification;
...
class ShareController extends Controller
{
    ...
    public function listPackage(Request $request): JsonResponse
    {
        ...
        $query->where('dpr.revision_status', '<>', 'draft')
    ...
    public function getHistory(Request $request): JsonResponse
    {
        ...
        $formatted = $logs->map(function ($log) {
            ...
            return [
                ...
                'time'       => Carbon::parse($log->created_at)->locale('en')->diffForHumans(),
    ...
    public function kpi(Request $request): JsonResponse
    {
        $query = DB::table('doc_package_revisions as dpr')
            ...
            ->join('customers as c', 'dp.customer_id', '=', 'c.id')
            ->join('models as m', 'dp.model_id', '=', 'm.id')
            ->join('doctype_groups as dp_dtg', 'dp.doctype_group_id', '=', 'dp_dtg.id')
            ->leftJoin('doctype_subcategories as dsc', 'dp.doctype_subcategory_id', '=', 'dsc.id')
            ->leftJoin('project_status as ps', 'm.status_id', '=', 'ps.id')
            ->where('dpr.revision_status', 'approved');
        ...
        if ($request->filled('doc_type') && $request->doc_type !== 'All') {
            $query->where('dp_dtg.name', $request->doc_type);
        }
        if ($request->filled('category') && $request->category !== 'All') {
            $query->where('dsc.name', $request->category);
        }
        ...
        $pendingRequest = DB::table('package_shares')
            ->whereIn('revision_id', $revisionIds)
            ->where('reshare_status', 'pending')
            ->count();

        $totalSuppliers = DB::table('suppliers')->where('is_active', 1)->count();
    ...
    public function getReshareRequests(Request $request): JsonResponse
    {
        $status = $request->get('status', 'pending'); // pending, approved, rejected, all

        $query = DB::table('package_shares as ps')
            ...
            ->select(
                ...
                'ps.reshare_status',
                'ps.reshare_reason',
                'ps.reshare_requested_at',
                'ps.reshare_reject_reason',
                'ps.reshare_processed_at',
                'ps.expired_at'
            );

        if ($status !== 'all') {
            $query->where('ps.reshare_status', $status);
        } else {
            $query->whereNotNull('ps.reshare_status');
        }

        $requests = $query->orderBy('ps.reshare_requested_at', 'desc')->get();

        return response()->json($requests);
    }

    public function approveReshare(Request $request): JsonResponse
    {
        $id = $request->input('id');
        $expiryDays = $request->input('expiry_days', 14);

        $share = DB::table('package_shares')->where('id', $id)->first();
        if (!$share) {
        ...
        try {
            DB::table('package_shares')
                ...
                ->update([
                    ...
                    'reshare_status' => 'approved',
                    'reshare_processed_at' => now(),
                    'updated_at' => now()
                ]);
            ...
            try {
                ActivityLog::create([
                    ...
                    'activity_code' => 'APPROVE_RESHARE',
                    'scope_type' => 'revision',
                    'scope_id' => $share->revision_id,
                    'revision_id' => $share->revision_id,
                    'meta' => [
                        'share_id' => $id,
                        'expiry_days' => $expiryDays,
                        'supplier_id' => $share->supplier_id
                    ]
                ]);
            } catch (\Exception $e) {
                Log::warning('ActivityLog failed for APPROVE_RESHARE: ' . $e->getMessage());
            }
            ...
            try {
                $package = DB::table('doc_package_revisions as dpr')
                    ...
                    ->leftJoin('doctype_subcategories as dsc', 'dp.doctype_subcategory_id', '=', 'dsc.id')
                    ->leftJoin('part_groups as pg', 'dp.part_group_id', '=', 'pg.id')
                    ->where('dpr.id', $share->revision_id)
                    ->select(
                        'c.code as customer',
                        'm.name as model',
                        'p.part_no',
                ...
                $packageName = $package ? (array) $package : [];
                $supplierName = DB::table('suppliers')->where('id', $share->supplier_id)->value('name') ?? 'Supplier';
                $newExpiry = now()->addDays($expiryDays)->format('Y-m-d');

                // Get all users associated with this supplier
                $supplierUsers = DB::table('users')
                    ->join('user_supplier', 'users.id', '=', 'user_supplier.user_id')
                    ->where('user_supplier.supplier_id', $share->supplier_id)
                    ->whereNotNull('users.email')
                    ->select('users.email', 'users.name')
                    ->get();

                $appUrl = url('/receipts/' . Crypt::encrypt($share->revision_id));

                foreach ($supplierUsers as $su) {
                    Mail::to($su->email)->send(new ReshareProcessedNotification(
                        $su->name,
                        $packageName,
                        'approved',
                        $newExpiry,
                ...
                Log::error('Reshare approved email notification failed: ' . $e->getMessage());
            }

            return response()->json(['message' => 'Request approved successfully. Access extended.']);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function rejectReshare(Request $request): JsonResponse
    {
        $id = $request->input('id');
        $reason = $request->input('reason');

        if (!$reason) {
            return response()->json(['message' => 'Reject reason is required.'], 422);
        }
        ...
        try {
            DB::table('package_shares')
                ->where('id', $id)
                ->update([
                    'reshare_status' => 'rejected',
                    'reshare_reject_reason' => $reason,
                    'reshare_processed_at' => now(),
                    'updated_at' => now()
                ]);
            ...
            try {
                ...
                Log::warning('ActivityLog failed for REJECT_RESHARE: ' . $e->getMessage());
            }
            ...
            try {
                ...
                Log::error('Reshare rejected email notification failed: ' . $e->getMessage());
            }
