...
use App\Mail\ReshareRequestNotification;
use App\Models\User;
use ZipArchive;
use Imagick;
use ImagickDraw;
...
class ReceiptController extends Controller
{
    ...
    public function getRevisionDetailJson(Request $request, $id)
    {
        ...
        // Cek Expired
        $isExpired = $shareInfo->expired_at && Carbon::parse($shareInfo->expired_at)->isPast();
        ...
        return response()->json([
            ...
            'isExpired'      => $isExpired,
            'reshareStatus'  => $shareInfo->reshare_status,
        ]);
    }
    ...
    private function _createStampImage(string $centerText, string $topLine, string|array $bottomLine, string $colorMode = 'blue'): ?\Imagick
    {
        try {
            ...
            // C. Center Text
            $draw->setFontSize(24); // Increased from 21
            ...
            // Auto-scale Top Line
            $metricsT = $stamp->queryFontMetrics($draw, $topLine);
            if ($metricsT['textWidth'] > ($canvasWidth - 10)) {
                $scale = ($canvasWidth - 10) / $metricsT['textWidth'];
                $draw->setFontSize($fontSizeTop * $scale);
            }
            ...
            if (is_array($bottomLine)) {
                ...
                // Auto-scale Bottom Line
                $metricsB = $stamp->queryFontMetrics($draw, $bottomLine);
                if ($metricsB['textWidth'] > ($canvasWidth - 10)) {
                    $scale = ($canvasWidth - 10) / $metricsB['textWidth'];
                    $draw->setFontSize($fontSizeBottom * $scale);
                }
    ...
    private function _burnStamps(string $originalPath, DocPackageRevisionFile $file, object $revision, object $package, ?StampFormat $stampFormat, array $extraData = []): string
    {
        ...
        try {
            ...
            if ($isObsolete) {
                ...
                $bottomLineObsolete = ["Name : {$obsName}", "Dept. : {$obsDept}"];
    ...
    public function requestReshare(Request $request): JsonResponse
    {
        $userId = Auth::user()->id;
        $userSupplier = DB::table('user_supplier')->where('user_id', $userId)->first();
        
        if (!$userSupplier) {
            return response()->json(['message' => 'Unauthorized. Only suppliers can request re-share.'], 403);
        }
        ...
        if (!$revisionId || !$reason) {
            return response()->json(['message' => 'Revision ID and Reason are required.'], 422);
        }

        $share = DB::table('package_shares')
            ->where('revision_id', $revisionId)
            ->where('supplier_id', $supplierId)
            ->first();

        if (!$share) {
            return response()->json(['message' => 'Share record not found.'], 404);
        }

        // Check if actually expired
        $isExpired = $share->expired_at && \Carbon\Carbon::parse($share->expired_at)->isPast();
        if (!$isExpired) {
            return response()->json(['message' => 'This package is not expired yet.'], 400);
        }

        DB::table('package_shares')
            ->where('revision_id', $revisionId)
            ->where('supplier_id', $supplierId)
            ->update([
                'reshare_requested_at' => now(),
                'reshare_status' => 'pending',
                'reshare_reason' => $reason,
                'reshare_reject_reason' => null,
                'updated_at' => now()
            ]);

        // Activity Log - Wrapped in try-catch to prevent constraint errors from breaking the request
        try {
            ActivityLog::create([
            'user_id' => $userId,
            'activity_code' => 'REQUEST_RESHARE',
            'scope_type' => 'revision',
            'scope_id' => $revisionId,
            ...
            Log::warning('ActivityLog failed for REQUEST_RESHARE: ' . $e->getMessage());
        }

        // Send Notification Email to PUD (Purchasing)
        try {
            $package = DB::table('doc_package_revisions as dpr')
                ->join('doc_packages as dp', 'dpr.package_id', '=', 'dp.id')
                ->join('customers as c', 'dp.customer_id', '=', 'c.id')
                ->join('models as m', 'dp.model_id', '=', 'm.id')
                ->join('products as p', 'dp.product_id', '=', 'p.id')
            ...
            $packageData = $package ? (array) $package : [];
            $supplierName = DB::table('suppliers')->where('id', $supplierId)->value('name') ?? 'Supplier';
            
            // Target PUD users
            $pudDeptId = DB::table('departments')->where('code', 'PUD')->value('id');
            $pudUsers = User::where('id_dept', $pudDeptId)->whereNotNull('email')->get();
            $requestUrl = route('file-manager.share'); // Standard sharing monitoring page where PUD handles requests

            foreach ($pudUsers as $pudUser) {
                Mail::to($pudUser->email)->send(new ReshareRequestNotification(
                    $supplierName,
                    $packageData,
                    $reason,
                    $requestUrl
            ...
            Log::error('Reshare request email notification failed: ' . $e->getMessage());
        }

        return response()->json(['message' => 'Success! Your request has been sent to purchasing.']);
    }
