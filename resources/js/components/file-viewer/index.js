...
export default (config = {}) => ({
    ...
    // Event handler for external file selection
    loadFile(file, force = false) {
        if (!file) return;

        // If it's a new file, the watcher on 'selectedFile' will trigger reloadFile()
        if (this.selectedFile?.name !== file.name) {
            this.selectedFile = file;
        } else if (force) {
            // If same file but forced (e.g., Try Again button), call reload explicitly
            this.reloadFile();
        }
    },
    ...
    reloadFile() {
        ...
        // 1. Reset all states first (Full Clean Slate)
        this.resetAllViewerStates();

        // 2. Identify and set loading for the specific viewer
        if (this.isPdf(name)) {
            this.pdfLoading = true;
            this.loadPdf(file);
        } else if (this.isTiff(name)) {
            this.tifLoading = true;
            this.loadTiff(file);
        } else if (this.isHpgl(name)) {
            this.hpglLoading = true;
            this.loadHpgl(file);
        } else if (this.isCad(name)) {
            // CAD uses internal state
            this.loadCad(file);
        } else {
    ...
    resetAllViewerStates() {
        ...
        // Reset all viewer-specific loading/error states
