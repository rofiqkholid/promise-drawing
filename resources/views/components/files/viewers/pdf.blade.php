@props(['selectedFile', 'isFullscreen', 'isCreatingMask', 'isPanning', 'enableMasking', 'masks', 'imageZoom', 'pkg', 'isStampBurned', 'stampConfig', 'isEngineering', 'pdfLoading', 'pdfError'])
...
<template x-if="isPdf(selectedFile?.name)">
    <div x-ref="viewport2d" class="relative w-full overflow-hidden bg-black/5 rounded"
        ...
        <div class="w-full h-full flex items-center justify-center">
            <div class="relative inline-block" :style="imageTransformStyle ? Object.assign(imageTransformStyle(), { transition: getTransitionStyle ? getTransitionStyle() : 'none' }) : {}">
        ...
        <div x-show="pdfLoading"
            ...
            <div class="relative w-12 h-12 sm:w-16 sm:h-16 mb-4 sm:mb-6">
                ...
                <div class="absolute inset-0 border-[3px] sm:border-4 border-red-500 rounded-full border-t-transparent animate-spin"></div>
                <i class="fa-solid fa-file-pdf absolute inset-0 m-auto w-fit h-fit text-red-500 animate-pulse text-lg sm:text-xl"></i>
            </div>

            <h3 class="text-xs sm:text-lg font-bold text-gray-800 dark:text-gray-100 mb-1.5 sm:mb-2">Loading PDF</h3>
            
            {{-- Progress Bar --}}
            <div class="w-40 sm:w-64 h-1 sm:h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mb-2">
                <div class="h-full bg-red-500 rounded-full transition-all duration-300 relative overflow-hidden" 
