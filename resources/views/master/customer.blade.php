...
@section('header-title', 'Customer Master')
...
<div class="p-4 sm:p-6 lg:p-8 text-gray-900 dark:text-gray-100">
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 sm:text-3xl">Customer Master</h2>
            ...
            <table id="customersTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 sorting" data-column="name">Customer Name</th>
                        <th scope="col" class="px-6 py-3 sorting" data-column="code">Customer Code</th>
    ...
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative p-4 text-left bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form id="addCustomerForm" action="{{ route('customers.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Customer Name <span class="text-red-600">*</span></label>
                ...
                <div class="mb-4">
                    <label for="code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Customer Code <span class="text-red-600">*</span></label>
...
{{-- Edit Customer Modal --}}
<div id="editCustomerModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full bg-black bg-opacity-50">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative p-4 text-left bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button type="button" class="close-modal-button text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                <i class="fa-solid fa-xmark w-5 h-5"></i>
            ...
            <h3 class="mb-4 text-xl text-center font-medium text-gray-900 dark:text-white">Edit Customer</h3>
            <form id="editCustomerForm" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="edit_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Customer Name <span class="text-red-600">*</span></label>
                ...
                <div class="mb-4">
                    <label for="edit_code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Customer Code <span class="text-red-600">*</span></label>
...
<div id="deleteCustomerModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full bg-black bg-opacity-50">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        toastError('Error', data.message || 'Failed to add customer.');
                    }
                },
                error: function(xhr) {
                    ...
                    const msg = xhr.responseJSON?.message || 'Failed to add customer.';
        ...
        $(document).on('click', '.edit-button', function() {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    const msg = xhr.responseJSON?.message || 'Failed to fetch customer data.';
                    toastError('Error', msg);
                },
                complete: function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        table.ajax.reload();
                        hideModal(editModal);
                        toastSuccess('Success', 'Customer updated successfully.');
                    } else {
                        toastError('Error', data.message || 'Failed to update customer.');
                    }
                },
                error: function(xhr) {
                    const errors = xhr.responseJSON?.errors;
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        toastError('Error', data.message || 'Failed to delete customer.');
                    }
                },
                error: function(xhr) {
                    const msg = xhr.responseJSON?.message || 'Failed to delete customer.';
                    toastError('Error', msg);
                },
                complete: function() {
                    setButtonLoading($btn, false);
                    setFormBusy($('#deleteCustomerModal'), false);
                }
            });
        });
