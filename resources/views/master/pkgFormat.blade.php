...
@section('header-title', 'Package Format Master')
...
<div class="p-4 sm:p-6 lg:p-8 text-gray-900 dark:text-gray-100">
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 sm:text-3xl">Package Format Master</h2>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md sm:rounded-lg overflow-hidden">
        <div class="p-4 md:p-6 overflow-x-auto">
            <table id="pkgFormatsTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
...
{{-- Add Package Format Modal --}}
<div id="addPkgFormatModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full bg-black bg-opacity-50">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Add New Package Format</h3>
            <form id="addPkgFormatForm" action="{{ route('pkg_format.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div class="mb-4">
                        <label for="prefix" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Prefix <span class="text-red-600">*</span></label>
                    ...
                    <div class="mb-4">
                        <label for="suffix" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Suffix <span class="text-red-600">*</span></label>
...
{{-- Edit Package Format Modal --}}
<div id="editPkgFormatModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full bg-black bg-opacity-50">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button type="button" class="close-modal-button text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                <i class="fa-solid fa-xmark w-5 h-5"></i>
            ...
            <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Package Format</h3>
            <form id="editPkgFormatForm" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div class="mb-4">
                        <label for="edit_prefix" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Prefix <span class="text-red-600">*</span></label>
                    ...
                    <div class="mb-4">
                        <label for="edit_suffix" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Suffix <span class="text-red-600">*</span></label>
...
<div id="deletePkgFormatModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full bg-black bg-opacity-50">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button type="button" class="close-modal-button text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                <i class="fa-solid fa-xmark w-5 h-5"></i>
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this package format?</p>
...
<style>
    ...
    input[type="search"][aria-controls="pkgFormatsTable"] {
        font-size: 0.75rem;
        line-height: 1rem;
...
<script>
    $(document).ready(function() {
        ...
        const table = $('#pkgFormatsTable').DataTable({
            processing: true,
            serverSide: true,
            scrollX: true,
            ajax: {
                url: '{{ route("pkg_format.data") }}',
                type: 'GET',
                data: function(d) {
            ...
            language: {
                emptyTable: '<div class="text-gray-500 dark:text-gray-400">No package formats found.</div>'
            },
            responsive: true,
            autoWidth: false,
        });
        ...
        // Modal Handling
        const addModal = $('#addPkgFormatModal');
        const editModal = $('#editPkgFormatModal');
        const deleteModal = $('#deletePkgFormatModal');
        const addButton = $('#add-button');
        const closeButtons = $('.close-modal-button');
        let pkgFormatIdToDelete = null;
        ...
        // Add Stamp Format
        $('#addPkgFormatForm').on('submit', function(e) {
            e.preventDefault();
            const $form = $(this);
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        $form[0].reset();
                        $('#is_active').prop('checked', true);
                        toastSuccess('Success', 'Package format added successfully.');
                    } else {
                        toastError('Error', data.message || 'Failed to add package format.');
                    }
                },
                error: function(xhr) {
                    ...
                    const msg = xhr.responseJSON?.message || 'Failed to add package format.';
        ...
        $(document).on('click', '.edit-button', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#editPkgFormatForm').attr('action', `{{ url('/') }}/master/pkgFormat/${id}`);
                    showModal(editModal);
                },
                error: function(xhr) {
                    const msg = xhr.responseJSON?.message || 'Failed to fetch package format data.';
                    toastError('Error', msg);
                },
                complete: function() {
        ...
        // Edit Stamp Format
        $('#editPkgFormatForm').on('submit', function(e) {
            e.preventDefault();
            const $form = $(this);
            const $btn = $form.find('[type="submit"]');
            const prefixError = $('#edit-prefix-error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        table.ajax.reload();
                        hideModal(editModal);
                        toastSuccess('Success', 'Package format updated successfully.');
                    } else {
                        toastError('Error', data.message || 'Failed to update package format.');
                    }
                },
                error: function(xhr) {
                    const errors = xhr.responseJSON?.errors;
                    ...
                    const msg = xhr.responseJSON?.message || 'Failed to update package format.';
                    toastError('Error', msg);
                },
                complete: function() {
                    setButtonLoading($btn, false);
        ...
        $(document).on('click', '.delete-button', function() {
            pkgFormatIdToDelete = $(this).data('id');
            showModal(deleteModal);
        });

        // Delete Stamp Format - Execute
        $('#confirmDeleteButton').on('click', function() {
            if (!pkgFormatIdToDelete) return;
            const $btn = $(this);

            $.ajax({
                url: `{{ url('/') }}/master/pkgFormat/${pkgFormatIdToDelete}`,
                method: 'DELETE',
                headers: { 'X-CSRF-TOKEN': csrfToken },
                beforeSend: function() {
                    setButtonLoading($btn, true, 'Deleting...');
                    setFormBusy($('#deletePkgFormatModal'), true);
                },
                success: function(data) {
                    if (data.success) {
                        table.ajax.reload();
                        hideModal(deleteModal);
                        pkgFormatIdToDelete = null;
                        toastSuccess('Success', 'Package format deleted successfully.');
                    } else {
                        toastError('Error', data.message || 'Failed to delete package format.');
                    }
                },
                error: function(xhr) {
                    const msg = xhr.responseJSON?.message || 'Failed to delete package format.';
                    toastError('Error', msg);
                },
                complete: function() {
                    setButtonLoading($btn, false);
                    setFormBusy($('#deletePkgFormatModal'), false);
                }
            });
        });
