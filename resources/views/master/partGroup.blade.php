...
<style>
    ...
    div.dataTables_length select {
        font-size: .75rem; line-height: 1rem; padding: .25rem 1.25rem .25rem .5rem;
...
$(function () {
    ...
    const table = $('#partGroupsTable').DataTable({
        ...
        lengthMenu: [10, 25, 50],
        order: [[1, 'asc']],
        language: { emptyTable: '<div class="text-gray-500 dark:text-gray-400">No part groups found.</div>' },
        responsive: true,
        autoWidth: false,
    });
    ...
    $(document).on('click', '.close-modal-button', function(){ hideModal($(this).closest('[tabindex="-1"]')); });

    $('#addPartGroupForm').on('submit', function(e){
        e.preventDefault();
        const $form = $(this), $btn = $form.find('[type="submit"]');
        $('#add-customer_id-error,#add-model_id-error,#add-code_part_group-error,#add-code_part_group_desc-error').addClass('hidden');
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                } else { toastError('Error', data.message || 'Failed to add part group.'); }
            },
            error: (xhr) => {
    ...
    $('#editPartGroupForm').on('submit', function(e){
        e.preventDefault();
        const $form = $(this), $btn = $form.find('[type="submit"]');
        $('#edit-customer_id-error,#edit-model_id-error,#edit-code_part_group-error,#edit-code_part_group_desc-error').addClass('hidden');
        ...
        $.ajax({
            ...
            beforeSend: () => { setButtonLoading($btn, true, 'Saving...'); setFormBusy($form, true); },
            success: (data) => {
                if (data.success) { table.ajax.reload(); hideModal(editModal); toastSuccess('Success', 'Part group updated successfully.'); }
                else { toastError('Error', data.message || 'Failed to update part group.'); }
            },
            error: (xhr) => {
                const e = xhr.responseJSON?.errors || {};
    ...
    $('#confirmDeleteButton').on('click', function(){
        ...
        $.ajax({
            ...
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': csrfToken },
            beforeSend: () => { setButtonLoading($btn, true, 'Deleting...'); setFormBusy($('#deletePartGroupModal'), true); },
            success: (data) => {
                if (data.success) {
                    table.ajax.reload();
                ...
                } else { toastError('Error', data.message || 'Failed to delete part group.'); }
            },
            error: (xhr) => toastError('Error', xhr.responseJSON?.message || 'Failed to delete part group.'),
            complete: () => { setButtonLoading($btn, false); setFormBusy($('#deletePartGroupModal'), false); }
        });
    });
