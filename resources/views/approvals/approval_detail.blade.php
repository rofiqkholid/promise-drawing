  ...
  <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
    ...
    <li aria-current="page">
      <div class="flex items-center">
        ...
        <a href="{{ route('approval') }}" class="text-sm font-semibold px-2.5 py-0.5 hover:text-blue-600 rounded-xs">
          Approval
        </a>
      </div>
    </li>
...
<div
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6 items-start">
    ...
    <div class="lg:col-span-4 space-y-6">
      ...
      <!-- ===== Meta Card ===== -->
      <div x-ref="metaCard"
        class="self-start bg-white dark:bg-gray-800 rounded-xs shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
        <!-- Header -->
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-6 md:justify-between">
            ...
            @endphp
            <a href="{{ $backUrl }}"
              class="inline-flex items-center gap-2 justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-xs shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-800">
...
<template x-if="canShare()">
  <button @click="openShareModal()"
    ...
    <i class="fa-solid fa-share-nodes mr-2"></i> Share
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xs shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex items-center justify-between">
          ...
          <span class="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded-xs"
              ...
              <div class="relative flex gap-3">
                ...
                <div class="relative flex-shrink-0 mt-1">
                  <template x-if="item.action === 'uploaded'">
                    <div class="w-6 h-6 rounded-xs bg-blue-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-cloud-arrow-up text-blue-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'approved'">
                    <div class="w-6 h-6 rounded-xs bg-green-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-check text-green-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'rejected'">
                    <div class="w-6 h-6 rounded-xs bg-red-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-xmark text-red-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'rollbacked'">
                    <div class="w-6 h-6 rounded-xs bg-amber-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-rotate-left text-amber-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'downloaded'">
                    <div class="w-6 h-6 rounded-xs bg-gray-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-download text-gray-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action.includes('share')">
                    <div class="w-6 h-6 rounded-xs bg-blue-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-share-nodes text-blue-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'revise_confirm'">
                    <div class="w-6 h-6 rounded-xs bg-purple-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-pen-to-square text-purple-600 text-xs"></i></div>
                  </template>
                  <template x-if="item.action === 'submit_approval'">
                    <div class="w-6 h-6 rounded-xs bg-yellow-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-paper-plane text-yellow-600 text-xs"></i></div>
                  </template>
                  ...
                  <template x-if="!['uploaded','approved','rejected','rollbacked','downloaded','revise_confirm','submit_approval'].includes(item.action) && !item.action.includes('share')">
                    <div class="w-6 h-6 rounded-xs bg-gray-100 flex items-center justify-center ring-4 ring-white dark:ring-gray-800 z-10 relative"><i class="fa-solid fa-circle-info text-gray-500 text-xs"></i></div>
                  </template>
                </div>

                <div class="flex-1 min-w-0" :class="idx !== (pkg.activityLogs || []).length - 1 ? 'mb-6' : ''">
                  <div class="p-3 rounded-xs bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <template x-if="item.snapshot && (item.snapshot.part_no || item.snapshot.ecn_no)">
                      <div class="mt-2 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xs text-xs shadow-sm">

                        <div class="flex items-center gap-2 mb-1">
                          <span class="font-bold text-gray-800 dark:text-gray-200" x-text="item.snapshot.part_no || '-'"></span>
                          <span class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded-xs font-mono text-[10px] border border-gray-200 dark:border-gray-600">
                            Rev <span x-text="item.snapshot.revision_no ?? '-'"></span>
                          </span>
                          <template x-if="item.snapshot.ecn_no">
                            <span class="text-blue-600 dark:text-blue-400 font-mono text-[10px] bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-xs border border-blue-100 dark:border-blue-800"
    ...
    <div class="lg:col-span-8">
      <div class="bg-white dark:bg-gray-800 rounded-xs border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col" style="min-height: 600px;">
  ...
  <div x-show="showApproveModal"
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xs shadow-xl w-full max-w-md mx-4 overflow-hidden">
      ...
      <div class="px-5 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        <button @click="closeApproveModal()" class="px-3 py-1.5 rounded-xs border text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
          Cancel
        </button>
        <button @click="confirmApprove()" :disabled="processing"
          class="px-3 py-1.5 rounded-xs bg-green-600 text-white text-sm hover:bg-green-700 disabled:opacity-60">
  ...
  <div x-show="showRejectModal"
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xs shadow-xl w-full max-w-lg mx-4 overflow-hidden">
      <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
      ...
      <div class="px-5 pb-2">
        <textarea x-model.trim="rejectNote" rows="4" placeholder="Enter rejection note here..."
          class="w-full rounded-xs border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm p-3 focus:outline-none focus:ring-2 focus:ring-red-500"></textarea>
        <p class="mt-1 text-xs text-red-600" x-show="rejectNoteError">Note is required</p>
      </div>

      <div class="px-5 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        <button @click="closeRejectModal()" class="px-3 py-1.5 rounded-xs border text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
          Cancel
        </button>
        <button @click="confirmReject()"
          :disabled="processing || rejectNote.length === 0"
          class="px-3 py-1.5 rounded-xs bg-red-600 text-white text-sm hover:bg-red-700 disabled:opacity-60">
  ...
  <div x-show="showRollbackModal"
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xs shadow-xl w-full max-w-md mx-4 overflow-hidden">
      ...
      <div class="px-5 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        <button @click="closeRollbackModal()" class="px-3 py-1.5 rounded-xs border text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
          Cancel
        </button>
        <button @click="confirmRollback()" :disabled="processing"
          class="px-3 py-1.5 rounded-xs bg-amber-600 text-white text-sm hover:bg-amber-700 disabled:opacity-60">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xs shadow-xl w-full max-w-lg mx-4 overflow-hidden">
      ...
      <div class="px-5 py-4 space-y-3 text-sm text-gray-700 dark:text-gray-200">
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            class="mt-1 p-2 block w-full rounded-xs border border-gray-300 dark:border-gray-600
      ...
      <div class="px-5 py-3 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        <button @click="closeShareModal()"
          class="px-3 py-1.5 rounded-xs border text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
          Cancel
        </button>
        <button @click="confirmShare()"
          :disabled="shareProcessing || shareNote.length === 0"
          class="inline-flex items-center gap-2 px-3 py-1.5 text-sm rounded-xs border border-blue-600
  ...
  function approvalDetail() {
    ...
    return {
      ...
      async onStampChange() {
        ...
        if (this.selectedFile && typeof this.isCad === 'function' && this.isCad(this.selectedFile.name)) {
            // CAD uses HTML overlay
        } else {
            // Image/PDF might need canvas redraw
            if (typeof this.redrawCanvas === 'function') this.redrawCanvas();
        }
        ...
        try {
          ...
          // CRITICAL: Update local data to keep it in sync without refresh
          if (this.selectedFile) {
            ...
            // Update in the file list array as well
            const groups = this.pkg.files || {};
            for (const category in groups) {
              const fileInList = groups[category].find(f => f.id === this.selectedFile.id);
              if (fileInList) {
                fileInList.ori_position = payload.ori_position;
                fileInList.copy_position = payload.copy_position;
                fileInList.obslt_position = payload.obslt_position;
                break;
              }
            }
          }
          ...
          for (const groupKey of Object.keys(groups)) {
            ...
            for (const file of list) {
              ...
              try {
                ...
                if (res.ok) {
                  successCount++;
                  // Update locally so switching files doesn't show old positions
      ...
      isWaiting() {
        ...
        return s === 'waiting checked' || s === 'waiting approved' || s === 'waiting legalize';
      },
      isApproved() {
      ...
      currentWaitingLevel() {
        const s = (this.pkg.status || '').toLowerCase();
        if (s === 'waiting checked') return 1;
        if (s === 'waiting approved') return 2;
        if (s === 'waiting legalize') return 3;
        return 0;
      },
     canAct() {
    if (!this.isWaiting()) return false;
    
    const currentStage = this.currentWaitingLevel(); // Tahapan dokumen saat ini (1, 2, atau 3)

    // JIKA ICT (Level 4): Selalu boleh Approve/Reject di tahap mana saja (Bypass)
    if (this.approvalLevel === 4) {
        return true;
    }

    // JIKA USER BIASA (Level 1, 2, 3): Hanya boleh Approve jika levelnya sesuai antrean
    return this.approvalLevel === currentStage;
},
      canRollback() {
    const s = (this.pkg.status || '').toLowerCase();

    // L1 bisa tarik jika sedang nunggu L2
    if (s === 'waiting approved' && this.approvalLevel === 1) return true;
