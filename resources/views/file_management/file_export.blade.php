        ...
        <div class="mt-4 sm:mt-6 sm:bg-white sm:dark:bg-gray-800 p-0 sm:px-6 sm:py-6 sm:rounded-sm border-0 sm:border sm:border-gray-200 sm:dark:border-gray-700 transition-all duration-300">
            ...
            <div class="flex flex-col items-center mb-6">
                <div class="relative w-full max-w-3xl group">
                    ...
                    <div class="absolute inset-y-0 right-0 pr-4 flex items-center gap-3">
                        <div class="hidden sm:flex items-center pointer-events-none text-gray-400 text-[10px] font-bold border border-gray-200 dark:border-gray-700 rounded px-1.5 py-0.5 group-focus-within:hidden">/</div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-12 gap-3 sm:gap-4 items-end">
                ...
                <div class="col-span-2 md:col-span-2 flex items-center gap-2">
                    ...
                    <button id="btnDownloadSummary" type="button" class="flex-1 h-9 sm:h-[38px] inline-flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold rounded-sm bg-emerald-600 text-white hover:bg-emerald-700 active:bg-emerald-800 transition-all">
...
<script>
    $(function() {
        ...
        function initTable() {
                ...
                `;
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            let viewButton = `
                <button type="button" title="View Package Details" class="w-10 h-10 flex items-center justify-center rounded-sm bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200 border border-transparent" onclick="openPackageDetails('${row.id}')">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-blue-500" viewBox="0 0 24 24" fill="none">
                        ...
                        <path d="M10.4 3.60001L12 6H21C21.6 6 22 6.4 22 7V19C22 19.6 21.6 20 21 20H3C2.4 20 2 19.6 2 19V4C2 3.4 2.4 3 3 3H9.20001C9.70001 3 10.2 3.20001 10.4 3.60001ZM11.7 16.7L16.7 11.7C17.1 11.3 17.1 10.7 16.7 10.3C16.3 9.89999 15.7 9.89999 15.3 10.3L11 14.6L8.70001 12.3C8.30001 11.9 7.69999 11.9 7.29999 12.3C6.89999 12.7 6.89999 13.3 7.29999 13.7L10.3 16.7C10.5 16.9 10.8 17 11 17C11.2 17 11.5 16.9 11.7 16.7Z" fill="currentColor"></path>
                            ...
                            let downloadButton = `
                <button type="button" title="Download Package" class="w-10 h-10 flex items-center justify-center rounded-sm bg-emerald-50 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700 transition-all duration-200 border border-transparent ml-1" onclick="confirmDownload('${row.id}')">
            ...
            // --- Table Event Listeners (Moved inside initTable to avoid 'undefined' crash) ---

            // Skeleton Trigger
            table.on('preXhr.dt', function() {
                $('#exportTable tbody').html(renderSkeletons(8));
            });
                    ...
                    if (hasActiveFilters) {
                      ...
                      <td colspan="10" style="padding: 0 !important; border: none !important; text-align: center !important;">
                          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; padding: 4rem 1rem; text-align: center !important;">
                              <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-sm mb-4 ring-1 ring-gray-100 dark:ring-gray-700">
                              ...
                              <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; justify-content: center; margin-bottom: 1.5rem; max-width: 48rem; padding: 0 1rem;">
                                  ${f.customer !== 'All' ? '<span class="px-3 py-1 text-xs font-semibold bg-gray-100 text-gray-700 border border-gray-200 rounded-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Customer: ' + ($('#customer').select2('data')[0]?.text || 'Selected') + '</span>' : ''}
                                  ${f.model !== 'All' ? '<span class="px-3 py-1 text-xs font-semibold bg-gray-100 text-gray-700 border border-gray-200 rounded-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Model: ' + ($('#model').select2('data')[0]?.text || 'Selected') + '</span>' : ''}
                                  ${f.doc_type !== 'All' ? '<span class="px-3 py-1 text-xs font-semibold bg-gray-100 text-gray-700 border border-gray-200 rounded-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Type: ' + ($('#document-type').select2('data')[0]?.text || 'Selected') + '</span>' : ''}
                                  ${f.category !== 'All' ? '<span class="px-3 py-1 text-xs font-semibold bg-gray-100 text-gray-700 border border-gray-200 rounded-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Category: ' + ($('#category').select2('data')[0]?.text || 'Selected') + '</span>' : ''}
                                  ${f.project_status !== 'All' ? '<span class="px-3 py-1 text-xs font-semibold bg-gray-100 text-gray-700 border border-gray-200 rounded-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Status: ' + ($('#project-status').select2('data')[0]?.text || 'Selected') + '</span>' : ''}
                              </div>
                              <button type="button" onclick="$('#btnResetFilters').click()" 
                                  class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-bold rounded-sm transition-all focus:ring-4 focus:ring-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">
            ...
            let resizeTimeout_adjust;
            $(window).on('resize', function() {
                clearTimeout(resizeTimeout_adjust);
                resizeTimeout_adjust = setTimeout(function() {
                    if (table) {
                        table.columns.adjust().draw(false);
                    }
                }, 250);
            });
        } // Standardized Skeleton Loader logic
        function renderSkeletons(count = 8) {
            const row = `
        <tr class="animate-pulse border-b border-gray-50 dark:border-gray-800">
            <td class="px-4 py-4"><div class="h-3 bg-gray-200 dark:bg-gray-700 rounded-sm w-6 mx-auto"></div></td>
            <td class="px-4 py-4">
                <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded-sm w-48 mb-2"></div>
                <div class="h-3 bg-gray-100 dark:bg-gray-800 rounded-sm w-32"></div>
            </td>
            <td class="px-4 py-4"><div class="h-5 bg-gray-200 dark:bg-gray-700 rounded-sm w-20 mx-auto"></div></td>
            <td class="px-4 py-4"><div class="h-3 bg-gray-200 dark:bg-gray-700 rounded-sm w-16"></div></td>
            <td class="px-4 py-4">
                <div class="h-3 bg-gray-200 dark:bg-gray-700 rounded-sm w-24 mb-1"></div>
                <div class="h-2 bg-gray-100 dark:bg-gray-800 rounded-sm w-16"></div>
            </td>
            <td class="px-4 py-4"><div class="h-5 bg-gray-200 dark:bg-gray-700 rounded-sm w-12 mx-auto"></div></td>
            <td class="px-4 py-4"><div class="h-3 bg-gray-100 dark:bg-gray-800 rounded-sm w-full"></div></td>
            <td class="px-4 py-4"><div class="h-3 bg-gray-200 dark:bg-gray-700 rounded-sm w-16"></div></td>
            <td class="px-4 py-4"><div class="h-3 bg-gray-200 dark:bg-gray-700 rounded-sm w-12 ml-auto"></div></td>
            <td class="px-4 py-4"><div class="flex justify-center gap-2"><div class="h-10 w-10 bg-gray-100 dark:bg-gray-700 rounded-sm"></div><div class="h-10 w-10 bg-gray-100 dark:bg-gray-700 rounded-sm"></div></div></td>
        </tr>
    `;
        ...
        function bindHandlers() {
            $('#customer, #model, #document-type, #category, #project-status').on('change', refreshData);
            ...
            <button type="button" class="recent-tag px-3 py-1 rounded-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-[10px] font-bold text-gray-500 dark:text-gray-400 hover:border-blue-400 hover:text-blue-500 transition-all whitespace-nowrap" data-term="${t}">
                <i class="fa-solid fa-history mr-1 opacity-50"></i> ${t}
            </button>
        `).join('');

                html += `
            <button type="button" id="btn-clear-recent" class="w-6 h-6 inline-flex items-center justify-center rounded-sm bg-gray-100 dark:bg-gray-800 text-gray-400 hover:text-red-500 transition-all ml-1" title="Clear History">
        ...
        `;
            ...
            function saveFilterState() {
                const filters = {
                    ...
                    customer: {
                        id: $('#customer').val(),
                        text: $('#customer').select2('data')[0]?.text
                    },
                    model: {
                        id: $('#model').val(),
                        text: $('#model').select2('data')[0]?.text
                    },
                    doc_type: {
                        id: $('#document-type').val(),
                        text: $('#document-type').select2('data')[0]?.text
                    },
                    category: {
                        id: $('#category').val(),
                        text: $('#category').select2('data')[0]?.text
                    },
                    project_status: {
                        id: $('#project-status').val(),
                        text: $('#project-status').select2('data')[0]?.text
                    },
                };
        ...
        function renderActivityLogs(logs) {
            ...
            logs.forEach((l, index) => {
                ...
                if (m.part_no || m.ecn_no) {
                    contentHtml += `
                        <div class="mt-2 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-sm text-xs">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-bold text-gray-800 dark:text-gray-200">${m.part_no || '-'}</span>
                                <span class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded-sm font-mono text-[10px] border border-gray-200 dark:border-gray-600">
                                    Rev ${m.revision_no ?? '-'}
                                </span>
                                ${m.ecn_no ? `<span class="text-blue-600 dark:text-blue-400 font-mono text-[10px] bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-sm border border-blue-100 dark:border-blue-800">${m.ecn_no}</span>` : ''}
                            </div>
                ...
                const el = $(`
                    <div class="relative flex gap-3">
                        ...
                        <div class="relative flex-shrink-0 mt-1">
                             <div class="w-6 h-6 rounded-sm flex items-center justify-center ${colorClass} ring-4 ring-white dark:ring-gray-800 z-10 relative">
                                <i class="fa-solid ${icon} text-xs"></i>
                             </div>
                        </div>
                        
                        <div class="flex-1 min-w-0 mb-6 ${isLast ? 'mb-0' : ''}">
                            <div class="p-3 rounded-sm bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        window.openPackageDetails = function(id) {
            ...
            fetch(`{{ url('/files') }}` + '/' + encodeURIComponent(id))
                ...
                .then(json => {
                    ...
                    dialog.className = 'bg-white dark:bg-gray-800 rounded-xs max-w-4xl w-full overflow-hidden';
                  ...
                  <div class="p-5 max-h-[70vh] overflow-y-auto space-y-6">
                      <div class="p-4 rounded-xs bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700">
                      ...
                      <div>
                          ...
                          <div class="p-3 rounded-xs border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">${revisionNote || '<span class="italic text-gray-400">No note provided.</span>'}</div>
                      </div>
                  ...
                  <div class="p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                      <button id="pkg-close-btn-2" class="inline-flex items-center gap-2 justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-xs text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-600">
        ...
        window.confirmDownload = function(packageId) {
            ...
            const rowData = table.rows().data().toArray().find(r => r.id === packageId);
            if (!rowData) return;

            const url = `{{ route('export.prepare-zip', ['revision_id' => ':id']) }}`.replace(':id', packageId);

            window.dispatchEvent(new CustomEvent('open-download-zip', {
                detail: {
                    count: rowData.file_count || 0,
                    size: formatBytes(rowData.total_size),
                    url: url
                }
            }));
        }
